setwd("C:/Users/kbhandari/Desktop/Ad Hoc")
...
feature_engineering <- function(dep_timeframe, n_days_back){
  ...
  transactions <- fread("data.csv", data.table = F)
...
Y_valid <- validation$actual

library(xgboost)
dtrain  <- xgb.DMatrix(as.matrix(train[,-which(names(train) %in% c("CUSTID","actual"))]), label = Y_train)
dvalid  <- xgb.DMatrix(as.matrix(validation[,-which(names(validation) %in% c("CUSTID","actual"))]), label = Y_valid)
...
xgb_model <- xgb.train(params=best_param, dtrain, nrounds = 500, watchlist, print_every_n = 5, early_stopping_rounds = 4, nthread=4)

imp <- xgb.importance(colnames(train[,-which(names(train) %in% c("CUSTID","actual"))]), model=xgb_model)
save(imp, file = "imp.rda")

rm(data, dtrain, dvalid, train, validation, Y_train, Y_valid, watchlist)

save(xgb_model, file = "xgb_model.rda")

data <- feature_engineering(dep_timeframe = "2017-08-04", n_days_back = 0)

dtest  <- xgb.DMatrix(as.matrix(data[,-which(names(data) %in% c("CUSTID","actual"))]))
data$pred = predict(xgb_model, dtest)
df <- data[,c("CUSTID","pred")]
write.csv(df,"xgb_scores.csv")
